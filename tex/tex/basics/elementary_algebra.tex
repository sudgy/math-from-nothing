...
In the Rocq code, there are notably many more theorems than what are said here.
Those are for convenience when doing algebra in Rocq, but the theorems are so
...
the Rocq code and it is interesting to see how these basic ideas follow from the
axioms.
...
\begin{theorem} \label{square_nz}
    If multiplication has the zero property, then if $0 = xx$, then $0 = x$.
\end{theorem}
\begin{proof}
    This follows directly from the zero property.
\end{proof}
...
Note that in the Rocq code, the dual relation is rarely used, to cut down on the
...
\begin{theorem} \label{abs_reverse_tri2}
    For all $a$ and $b$, $|a| - |b| \leq |a - b|$.
\end{theorem}
\begin{proof}
...
\begin{theorem} \label{abs_reverse_tri}
    For all $a$ and $b$, $||a| - |b|| \leq |a - b|$.
\end{theorem}
\begin{proof}
