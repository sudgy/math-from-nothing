...
\begin{instance}
    ...
    (g : \Ord_\beta) \to \Ord$ that is equal to $\alpha$ if $\beta = 0$, or is
    equal to the smallest ordinal greater than all values of $g$.  Let $g$ be
    the function given by transfinite recursion on $f$.  Then define the sum
    $\alpha + \beta$ to be equal to $g(\beta)$.
\end{instance}
...
\begin{theorem} \label{ord_plus_ind}
    For all $\alpha$ and $\beta$ with $\beta \neq 0$, then $\alpha + \beta$ is
    the least element of the set $\{\gamma : \Ord \mid \forall \delta < \beta,
    \alpha + \delta < \gamma\}$.
\end{theorem}
\begin{proof}
...
\begin{theorem} \label{ord_plus_eq}
    For all $\alpha$, $\beta$, and $\varepsilon$ with $\beta \neq 0$, then if
    $\varepsilon$ is the least element of the set $\{\gamma : \Ord \mid \forall
    \delta < \beta, \alpha + \delta < \gamma\}$, we have $\varepsilon = \alpha +
    \beta$.
\end{theorem}
\begin{proof}
    This follows from the uniqueness of least elements.
\end{proof}
...
\begin{proof}
    ...
    transfinite induction.  The inductive hypothesis states that for all $\beta
    < \alpha$, we have $0 + \beta = \beta$.  When $\alpha = 0$, we have $0 + 0 =
    0$.  When $\alpha \neq 0$, we will use Theorem \ref{ord_plus_eq} to prove
    that $0 + \alpha = \alpha$.  To prove that $0 + \delta < \alpha$ for all
    $\delta < \alpha$, we can use $0 + \delta = \delta$ from the inductive
    hypothesis.  Now let $\gamma$ be some other ordinal such that for all
    $\delta < \gamma$, we have $0 + \delta < \gamma$.  If we had $\gamma <
    \alpha$, we would have $0 + \gamma < \gamma$ by the definition of $\gamma$,
    and by the inductive hypothesis we have $\gamma < \gamma$, which is
    impossible.  Thus, we must have $\alpha \leq \gamma$, showing that the
    conditions of Theorem \ref{ord_plus_eq} are satisfied.
\end{proof}
...
\end{theorem}
\begin{proof}
    Because $\alpha < \beta$, we have $\beta \neq 0$ by Theorem \ref{ord_pos2}.
    Then the result follows directly from Theorem \ref{ord_plus_ind}.
\end{proof}
...
\begin{theorem} \label{ord_le_ex}
    ...
    exists an ordinal $\gamma$ such that $\alpha + \gamma = \beta$.  If $\alpha
    = \beta$, then $\gamma = 0$ suffices, so now consider the case $\alpha \neq
    \beta$.  Let $S$ be the set $\{\varepsilon : \Ord \mid \beta \leq \alpha +
    \varepsilon\}$.  By Theorem \ref{ord_le_self_lplus}, this set is nonempty,
    so it has a least element $\gamma$.  If $\gamma = 0$, we would have $0 \in
    S$, so $\beta \leq \alpha$, contradicting $\alpha \leq \beta$ and $\alpha
    \neq \beta$.  So we must have $\gamma \neq 0$.  We will prove by Theorem
    \ref{ord_plus_eq} that $\alpha + \gamma = \beta$.
...
\end{theorem}
\begin{proof}
    If $0 = \alpha + \beta$, then by Theorem \ref{ord_plus_ind}, we have $\alpha
    + \delta < 0$ for all $\delta < \beta$.  Since $\beta \neq 0$, we have $0 <
    \beta$, so $\alpha + 0 < 0$, contradicting Theorem \ref{ord_pos}.
\end{proof}
...
\begin{proof}
    ...
    0$.  By Theorem \ref{ord_nz_rplus}, we have $0 \neq \beta + \gamma$.  We
    will prove the result using Theorem \ref{ord_plus_eq} with $\alpha + \beta$
    and $\gamma$.

    First, let $\delta < \gamma$.  Then
    \begin{align*}
        \delta &< \gamma \\
        ...
        (\alpha + \beta) + \delta &< \alpha + (\beta + \gamma),
    \end{align*}
    which is the first thing we need to prove.

    Now let $\varepsilon$ be an ordinal such that  such that for all $\delta <
    \gamma$, we have $(\alpha + \beta) + \delta < \varepsilon$.  We must prove
    that $\alpha + (\beta + \gamma) \leq \varepsilon$.  By Theorem
    \ref{ord_plus_ind}, it suffices to prove that for all $\delta < \beta +
    \gamma$, we have $\alpha + \delta < \varepsilon$.  There will be two cases:
    when $\beta \leq \delta$, and when $\delta < \beta$.

    When $\beta \leq \delta$, by Theorem \ref{ord_le_ex} we have an ordinal
    $\zeta$ with $\beta + \zeta = \delta$.  Because $\delta < \beta + \gamma$,
    we have $\beta + \zeta < \beta + \gamma$, and cancelling we get $\zeta <
    \gamma$.  Because $\zeta < \gamma$, by the definition of $\varepsilon$, we
    have $(\alpha + \beta) + \zeta < \varepsilon$, and by the inductive
    hypothesis we have $\alpha + (\beta + \zeta) < \varepsilon$.  Because $\beta
    + \zeta = \delta$, we have $\alpha + \delta < \varepsilon$ as required.

    When $\delta < \beta$, we have $\alpha + \delta < \alpha + \beta$.  By
    transitivity, it suffices to prove that $\alpha + \beta < \varepsilon$.  By
    the definition of $\varepsilon$, since $0 < \gamma$, we have $(\alpha +
    \beta) + 0 < \varepsilon$, so $\alpha + \beta < \varepsilon$ as required.
\end{proof}
