...
\begin{definition} \label{small_set_to_card}
...
\begin{theorem} \label{ord_normal_fixed_sup_eq}
    For all normal functions $f$ and ordinals $\alpha$, the first fixed point of
    $f$ greater than or equal to $\alpha$ as given by Theorem
    \ref{ord_normal_family_fixed_eq} is equal to
    \[
        \sup\{f^n(\alpha) \mid n < \omega\}.
    \]
\end{theorem}
\begin{proof}
    Upon unfolding all the definitions, this fact looks trivial, except for the
    fact that $F(\alpha)$ uses lists of just $f$, while the form given in this
    theorem uses iterating $f$.  In the Rocq code I had to do a bunch of stuff
    to show prove this, but they really are the same thing, so I won't show the
    proof here.
\end{proof}
