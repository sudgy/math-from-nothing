...
Definition ext_n_algebra_module := grade_sum nat ext_n_module.

Definition ext_n_grade := grade_sum_grade nat ext_n_module
    : GradedSpace U ext_n_algebra_module.
Local Existing Instances ext_n_grade.

Definition to_ext_n {k} (X : ext_n_module k)
    := single_to_grade_sum nat ext_n_module X : module_V ext_n_algebra_module.
...
Lemma ext_n_algebra_mult_in : ∀ m n a b,
    subspace_set (ext_n_subspace m) a → subspace_set (ext_n_subspace n) b
...
Definition ext_n_mult_base i j
    (a : module_V ext_n_algebra_module)
    (b : module_V ext_n_algebra_module)
    (ai : of_grade i a) (bj : of_grade j b)
    := to_ext_n
        [[ex_val ai|] * [ex_val bj|] |
            ext_n_algebra_mult_in i j [ex_val ai|] [ex_val bj|]
                [|ex_val ai] [|ex_val bj]].

Lemma ext_n_mult_tm : ∀ i j a b AH BH,
    ext_n_mult_base i j (to_ext_n a) (to_ext_n b) AH BH
    = to_ext_n [[a|] * [b|] |
        ext_n_algebra_mult_in i j [a|] [b|] [|a] [|b]].
Proof.
    intros i j a b AH BH.
    unfold ext_n_mult_base.
    rewrite_ex_val a' a'_eq.
    rewrite_ex_val b' b'_eq.
    unfold to_ext_n in a'_eq, b'_eq.
    apply single_to_grade_sum_eq in a'_eq, b'_eq.
    subst a' b'.
    reflexivity.
Qed.

Theorem ext_n_mult_base_ldist :
    bilinear_extend_ldist_base ext_n_mult_base.
Proof.
    intros u' v' w' i j iu jv jw.
    pose proof iu as [u u_eq].
    pose proof jv as [v v_eq].
    pose proof jw as [w w_eq].
    change (single_to_grade_sum nat ext_n_module)
        with (to_ext_n (k := i)) in u_eq.
    change (single_to_grade_sum nat ext_n_module)
        with (to_ext_n (k := j)) in v_eq, w_eq.
    subst u' v' w'.
    assert (of_grade j (to_ext_n (v + w))) as vwj.
    {
        unfold to_ext_n.
...
Theorem ext_n_mult_base_rdist :
    bilinear_extend_rdist_base ext_n_mult_base.
Proof.
    ...
    pose proof jw as [w w_eq].
    change (single_to_grade_sum nat ext_n_module)
        with (to_ext_n (k := i)) in u_eq, v_eq.
    change (single_to_grade_sum nat ext_n_module)
        with (to_ext_n (k := j)) in w_eq.
    subst u' v' w'.
    assert (of_grade i (to_ext_n (u + v))) as uvi.
    {
        unfold to_ext_n.
        rewrite single_to_grade_sum_plus.
        apply of_grade_plus; assumption.
...
Theorem ext_n_mult_base_lscalar :
    bilinear_extend_lscalar_base ext_n_mult_base.
Proof.
    ...
    change (single_to_grade_sum nat ext_n_module)
        with (to_ext_n (k := i)) in u_eq.
    change (single_to_grade_sum nat ext_n_module)
        with (to_ext_n (k := j)) in v_eq.
    subst u' v'.
    assert (of_grade i (to_ext_n (a · u))) as aui.
    {
        unfold to_ext_n.
...
Theorem ext_n_mult_base_rscalar :
    bilinear_extend_rscalar_base ext_n_mult_base.
Proof.
    intros a u' v' i j iu jv.
    pose proof iu as [u u_eq].
    pose proof jv as [v v_eq].
    ...
    change (single_to_grade_sum nat ext_n_module)
        with (to_ext_n (k := j)) in v_eq.
    subst u' v'.
    assert (of_grade j (to_ext_n (a · v))) as avj.
    {
        unfold to_ext_n.
        rewrite single_to_grade_sum_scalar.
        apply of_grade_scalar; assumption.
...
Instance ext_n_mult : Mult (module_V ext_n_algebra_module) := {
    mult A B := bilinear_extend ext_n_mult_base A B
}.
...
Proof.
    ...
    -   apply ext_n_mult_base_ldist.
    -   apply ext_n_mult_base_rscalar.
Qed.
Local Instance ext_n_mult_rdist : Rdist (module_V ext_n_algebra_module).
Proof.
    split.
    apply bilinear_extend_rdist.
    -   apply ext_n_mult_base_rdist.
    -   apply ext_n_mult_base_lscalar.
Qed.
...
Theorem to_ext_n_mult : ∀ m n a b
    ...
    to_ext_n [a * b|ext_n_algebra_mult_in m n a b ma nb].
Proof.
    intros m n a b ma nb.
    unfold mult at 1; cbn.
    assert (of_grade (H9 := ext_n_grade) m (to_ext_n [a|ma])) as ma'.
    {
        exists [a|ma].
        reflexivity.
    }
    assert (of_grade (H9 := ext_n_grade) n (to_ext_n [b|nb])) as nb'.
    {
        exists [b|nb].
        reflexivity.
    }
    rewrite (bilinear_extend_homo _
        ext_n_mult_base_ldist ext_n_mult_base_rdist
        ext_n_mult_base_lscalar ext_n_mult_base_rscalar _ _ _ _ ma' nb').
    rewrite ext_n_mult_tm; cbn.
    reflexivity.
Qed.

Instance ext_n_grade_mult : GradedAlgebraObj U (module_V ext_n_algebra_module).
Proof.
    split.
    ...
    pose proof (ext_n_algebra_mult_in _ _ _ _ u_in v_in) as uv_in.
    exists [_|uv_in].
    apply f_equal.
...
Proof.
    ...
    change (single_to_grade_sum nat ext_n_module v) with (to_ext_n v).
...
Proof.
    ...
    rewrite ulist_image_add, ulist_sum_add; cbn.
    change (grade_sum_type nat ext_n_module) with (module_V (ext_n_algebra_module)).
    do 3 rewrite rdist.
    rewrite IHl.
    apply rplus.
    clear l IHl.
...
Proof.
    ...
    unfold vector_to_ext_n.
    rewrite to_ext_n_mult.
    rewrite <- (single_to_grade_sum_zero _ _ 2).
    apply to_ext_n_eq.
