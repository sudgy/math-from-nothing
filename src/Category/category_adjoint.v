...
Class Adjunction `{C1 : Category, C2 : Category}
    ...
    adj_a : ∀ {A B}, cat_morphism (F A) B → cat_morphism A (G B);
    adj_bij : ∀ A B, Bijective (@adj_a A B);
    adj_b {A B} := bij_inv _ (f_bij := adj_bij A B);
    adj_nat1 : ∀ {A B C} (g : cat_morphism (F A) B)
                         (q : cat_morphism B C),
        adj_a (q ∘ g) = (⌈G⌉ q) ∘ adj_a g;
    adj_nat2 : ∀ {A B C} (p : cat_morphism A B)
                         (f : cat_morphism B (G C)),
...
Theorem adj_ab : ∀ {A B} (f : cat_morphism A (G B)), adj_a Ad (adj_b Ad f) = f.
Proof.
    intros A B f.
    unfold adj_b.
    apply inverse_eq2.
    apply bij_inv_inv.
Qed.
Theorem adj_ba : ∀ {A B} (f : cat_morphism (F A) B), adj_b Ad (adj_a Ad f) = f.
Proof.
    intros A B f.
    unfold adj_b.
    apply inverse_eq1.
...
Local Program Instance compose_adjunction
    ...
    : Adjunction (F' ○ F) (G ○ G') :=
{
    adj_a {A B} (f : cat_morphism (F' (F A)) B) := adj_a A1 (adj_a A2 f);
}.
Next Obligation.
