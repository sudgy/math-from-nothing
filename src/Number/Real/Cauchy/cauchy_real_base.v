...
Let real_eq (a b : real_base) :=
    ∀ ε, 0 < ε → ∃ N, ∀ i, N ≤ i → |a i - b i| < ε.

Local Infix "~" := real_eq.

(* This doesn't directly use real_eq because that takes in values of type
 * real_base when this is often applied to sequences that are not of that type
 *)
Lemma real_eq_zero : ∀ a b : nat → rat, (∀ i, a i - b i = 0) →
    ∀ ε, 0 < ε → ∃ N, ∀ i, N ≤ i → |a i - b i| < ε.
Proof.
    intros a b all_z ε ε_pos.
    exists 0.
    intros i i_ge.
    rewrite all_z.
    rewrite <- abs_zero.
...
Proof.
    split.
    intros a.
    unfold real_eq; apply real_eq_zero.
    intros i.
    apply plus_rinv.
Qed.
...
Notation "a ~ b" := (eq_equal real_equiv a b).
