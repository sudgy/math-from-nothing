...
Lemma rat_le_wd_1 : ∀ a b c d, a ~ b → c ~ d → a ≦ c → b ≦ d.
    ...
    cbn in *.
    apply le_rmult_pos with (nat1_to_int b2) in ac.
    2: apply nat1_to_int_pos.
    mult_bring_left (nat1_to_int b2) in ac.
    mult_bring_left a1 in ac.
    rewrite mult_assoc in ac.
    rewrite ab in ac.
    apply le_rmult_pos with (nat1_to_int d2) in ac.
    2: apply nat1_to_int_pos.
    mult_bring_left (nat1_to_int d2) in ac.
    mult_bring_left c1 in ac.
    rewrite (mult_assoc c1) in ac.
    rewrite cd in ac.
    mult_bring_right (nat1_to_int c2) in ac.
...
Lemma rat_le_transitive : ∀ a b c, a <= b → b <= c → a <= c.
    ...
    unfold le; equiv_simpl.
    intros ab bc.
    apply le_rmult_pos with (nat1_to_int c2) in ab.
    2: apply nat1_to_int_pos.
    apply le_rmult_pos with (nat1_to_int a2) in bc.
    2: apply nat1_to_int_pos.
    rewrite <- (mult_assoc b1) in ab.
    rewrite (mult_comm (nat1_to_int a2)) in ab.
    rewrite mult_assoc in ab.
    pose proof (trans ab bc) as eq.
    mult_bring_right (nat1_to_int b2) in eq.
...
Lemma rat_le_lplus : ∀ a b c, a <= b → c + a <= c + b.
    ...
    do 2 rewrite <- (nat1_to_int_mult _ (c2 * c2)).
